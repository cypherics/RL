...
class DeterministicSARSA:
  ...
  def policy(self, state, is_training):
    ...
    if np.random.random() > self.eps or not is_training:
  ...
  def train_step(self, state, action, reward, next_state, next_action, done):
    ...
    self.Q[state[0]][action] = self.alpha * (reward + self.gamma * self.expectation(next_state) - self.Q[state[0]][action])

  def expectation(self, state):
    sum = 0
    for action in range(0, self.num_actions):
      if not self.is_state_int(state):
        sum += state[1] * self.Q[state[0]][action]
      else:
        sum += 1 * self.Q[state][action]
    return sum
